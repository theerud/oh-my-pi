...
fn confstr_cs_path() -> Result<Option<String>, io::Error> {
	let value = confstr(nix::libc::_CS_PATH)?;

	if let Some(value) = value {
		let value_str = value
			.into_string()
			.map_err(|_err| io::Error::new(io::ErrorKind::InvalidData, "Invalid data"))?;
		Ok(Some(value_str))
	} else {
		Ok(None)
	}
...
fn confstr(name: nix::libc::c_int) -> Result<Option<std::ffi::OsString>, io::Error> {
	// SAFETY:
