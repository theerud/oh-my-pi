...
mod native_impl {
	...
	impl MatchCollector {
		...
		pub fn truncate_line(&self, line: &str) -> (String, bool) {
	...
	pub fn build_searcher(before_context: u32, after_context: u32) -> Searcher {
		SearcherBuilder::new()
	...
	pub fn to_public_match(matched: CollectedMatch) -> Match {
	...
	pub fn to_grep_match(path: &str, matched: CollectedMatch) -> GrepMatch {
		let context_before = if matched.context_before.is_empty() {
			None
		} else {
			Some(matched.context_before.into_vec())
	...
	pub const fn empty_search_result(error: Option<String>) -> SearchResult {
	...
	pub fn grep_sync(
		...
		let entries = if use_cache {
			...
			let fresh =
				fs_cache::force_rescan(&search_path, include_hidden, use_gitignore, false, &ct)?;
...
mod system_impl {
	...
	pub fn search_sync(_content: &[u8], _options: SearchOptions) -> SearchResult {
		SearchResult {
			...
			limit_reached: false,
			error:         Some(
				"System-based ripgrep does not support in-memory search. Use native build or grep()."
					.to_string(),
			),
		}
	}
	...
	pub fn grep_sync(
		...
		if options.multiline.unwrap_or(false) {
			args.push("--multiline".to_string());
		}
		if !options.hidden.unwrap_or(true) {
			// ripgrep hides hidden by default, we enable them by default.
		} else {
			args.push("--hidden".to_string());
		}
		if !options.gitignore.unwrap_or(true) {
		...
		for line in reader.lines() {
			let line =
				line.map_err(|e| Error::from_reason(format!("Error reading rg output: {e}")))?;
					...
					"match" => {
						if let Some(data) = msg.data {
							...
							let grep_match = GrepMatch {
								...
								line_number,
								line: line_text,
								context_before: None, // rg json output for context is separate
								context_after: None,
								truncated: None,
								match_count: None,
							};
							if let Some(cb) = on_match {
								cb.call(Ok(grep_match.clone()), ThreadsafeFunctionCallMode::NonBlocking);
							}
							matches.push(grep_match);
							total_matches += 1;
						}
...
mod no_impl {
	...
	pub fn search_sync(_content: &[u8], _options: SearchOptions) -> SearchResult {
		SearchResult {
			...
			match_count:   0,
			limit_reached: false,
			error:         Some("Text search is disabled in this build.".to_string()),
		}
	}
