...
export interface RootCommandSummary {
	name: string;
	description: string;
	aliases?: string[];
	hidden?: boolean;
...
export function renderTopLevelHelp(bin: string, version: string, commands: RootCommandSummary[]): void {
	const visibleCommands = commands.filter(command => !command.hidden);
	const maxLen = Math.max(...visibleCommands.map(command => command.name.length));
	const commandLines = visibleCommands
		.sort((left, right) => left.name.localeCompare(right.name))
		.map(command => {
			const aliasText = command.aliases?.length ? ` (${command.aliases.join(", ")})` : "";
			return `  ${command.name.padEnd(maxLen + 2)}${command.description}${aliasText}`;
		});
...
Default behavior starts the interactive coding agent.
Run ${bin} launch --help for launch-mode flags and options.
