...
export const hash = Object.assign((data: any) => simpleHash(data, "sha256"), {
	xxHash32: (data: any) => {
		let h = 0;
		const s = String(data);
...
export const Glob = class {
	...
	*scanSync(_options: any = {}) {
		yield* [];
	}
	async *scan(_options: any = {}) {
		yield* [];
	}
};
...
export const Archive = {
	async write(_outputPath: string, _data: any, _options: any) {
		// Placeholder
	},
};
...
class ShellPromise extends Promise<any> {
	...
	// biome-ignore lint/suspicious/noThenProperty: ShellPromise must be thenable to work with await
